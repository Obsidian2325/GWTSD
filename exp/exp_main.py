...
class Exp_Main(Exp_Basic):
    ...
    def _get_adj(self):
        adj = pd.read_csv(os.path.join(self.args.root_path, self.args.adj_path), header=0)
        adj_data = adj.values
        adj_mat = torch.tensor(adj_data, dtype=torch.float32)
    ...
    def _build_model(self):
        ...
        adj = self._get_adj()
        model = model_dict[self.args.model].Model(self.args, adj).float()
    ...
    def train(self, setting):
        ...
        print(f"[Train Info] Total training time: {total_time}")
        if torch.cuda.is_available():
            max_mem = torch.cuda.max_memory_allocated(device=self.device) / 1024 / 1024
            print(f"[Train Info] Peak GPU memory usage: {max_mem:.2f} MB")
